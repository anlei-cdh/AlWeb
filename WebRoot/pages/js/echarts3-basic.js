...
TerrenCharts.echarts.prototype = {
	...
	setLayoutOption: function(myChart, option) {
		var nodes = this.option.series[0].data;
		var links = this.option.series[0].links;
		for(var i=0;i<nodes.length;i++) {
			var node = nodes[i];
			node.symbol = 'circle';
			node.symbolSize = node.symbolSize2;
			node.value = node.symbolSize;
			if(node.category == 3 || node.category == 4 || node.category == 5) {
				node.label.normal.position = "left";
				if(node.category == 5 && node.name == "国际") {
					node.label.normal.position = "bottom";
				}
			}
			if(node.category == 2) {
				node.label.normal.position = "top";
				if(node.name == "社会") {
					node.label.normal.position = "left";
				}
			}
			if(node.category == 6) {
				if(node.name == "聚类6" || node.name == "国际") {
					node.label.normal.position = "bottom";
				}
			}
		}
		this.option.series[0].layout = "circular";
		this.option.series[0].lineStyle.normal.curveness = 0.4;
...
TerrenCharts.echarts.graphCharts.prototype.getchartsData = function(jsonList,jsonConfig) {
	...
	for(var i=0;i<nodes.length;i++) {
		...
		if(node.level == 0 && !jsonConfig.labels) {
			...
			var index = node.category % colorList.length;
			labels.push({name: node.name,color: colorList[index]});
		}
	}
...
TerrenCharts.echarts.graphCharts.prototype.initSeries = function(chartsData) {
    ...
    for(var i=0;i<labels.length;i++) {
    	var categorie = {name: labels[i].name,itemStyle: {normal: {color: labels[i].color}}};
    	categories.push(categorie);
    }
    this.legend.data = labels;
    this.legend.textStyle = {color: '#FFFFFF'}
